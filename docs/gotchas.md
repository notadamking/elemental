...
- **PTY terminal input requires `\r` not `\n`** — when writing to interactive PTY sessions (like the director), use carriage return (`\r`) to submit input, not newline (`\n`). The `messageSession` method sends the message content followed by a separate `\r` write to ensure proper input submission.
