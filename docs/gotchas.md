...
- **Closed-but-unmerged tasks are automatically reconciled** — if a task reaches CLOSED status but its `mergeStatus` is not in a terminal state (`'merged'` or `'not_applicable'`), the dispatch daemon moves it back to REVIEW after a grace period (default 120s). Tracked via `orchestrator.reconciliationCount` in metadata; stops after 3 attempts (safety valve). Configure with `closedUnmergedReconciliationEnabled` and `closedUnmergedGracePeriodMs`.
