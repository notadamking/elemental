...
Full-text search index for documents. Uses BM25 ranking and supports snippet generation. Populated via triggers on the `elements` table for document-type rows.
...
Stores vector embeddings for semantic search. Embeddings are generated by a local embedding provider and used for similarity-based document retrieval.
