...
- [x] **types/message.md Phase 4**: Implement `replies-to` dependency creation ✅ (integrated in ElementalAPI.create() with event recording, 4 tests in message-integration.test.ts)
- [x] **types/entity.md Phase 6**: Integrate with Task assignment ✅ (TaskLike interface, isValidTaskAssignee, validateTaskAssignee, getTasksAssignedTo, getTasksOwnedBy, getTasksCreatedBy, getTasksInvolvingEntity, countTasksAssignedTo, countTasksByAssignee, getTopTaskAssignees, hasTasksAssigned, getUnassignedTasks, getEntityTasksByStatus, getEntityTaskStats, filterEntitiesWithTasks, filterEntitiesWithoutTasks, filterEntitiesByTaskLoad, getAvailableAssignees, getEntityWorkloadDistribution, findLeastBusyEntity, getTaskCoworkers, countTaskCoworkers - 40+ tests in entity.test.ts)
- [x] **types/entity.md Phase 6**: Integrate with Message sender validation ✅ (isValidMessageSender, canSendToChannel, validateMessageSender, getMessagesSentBy, getEntityChannelMessages, countMessagesSentBy, countMessagesBySender, getTopMessageSenders, hasSentMessages, getMostRecentMessageBy, getChannelsWithMessagesFrom, getEntityMessageStats, filterEntitiesWithMessages, filterEntitiesWithoutMessages, getChannelParticipants, getMessagePartners, canCryptographicallySign, filterEntitiesWithSigningCapability, getVerifiedMessageSenders - 48 tests in entity.test.ts)
...
- [x] **types/workflow.md Phase 2 (remaining)**: Status change event emission ✅ (workflow status changes to terminal states emit 'closed' events, tested in events.integration.test.ts - 7 tests)
