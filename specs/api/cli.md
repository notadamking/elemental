...
- [x] **el-59p3** - CLI parser overwrites repeated options instead of accumulating to array. ✅ Fixed by adding `array?: boolean` property to `CommandOption` type and updating parser to accumulate values for array options. All repeatable options (`--tag`, `--step`, `--variable`, `--extends`, `--member`, `--attachment`, `--var`, `--add-tag`, `--remove-tag`) now have `array: true` set.
