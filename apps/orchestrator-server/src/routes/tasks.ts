...
export function createTaskRoutes(services: Services) {
  ...
  app.post('/api/tasks/:id/dispatch/smart', async (c) => {
    try {
      ...
      if (errorMessage.includes('No eligible agents') || errorMessage.includes('No available agents')) {
        return c.json({ error: { code: 'NO_ELIGIBLE_AGENTS', message: 'No eligible agents found' } }, 404);
      }
    ...
    try {
      ...
      return c.json({
        ...
        bestCandidate: result.bestCandidate
          ? { agent: { id: result.bestCandidate.agent.id, name: result.bestCandidate.agent.name } }
          : null,
      });
    } catch (error) {
