...
function getActivityClass(level: number): string {
  const baseClass = 'w-3 h-3 rounded-sm transition-colors';
  switch (level) {
    case 0:
...
export function ContributionChart({
  ...
  }, [activity, days, startDate, endDate]);
  ...
  return (
    <div className="relative" data-testid={testId}>
      {/* Month labels */}
      <div className="flex ml-8 mb-1 text-xs text-gray-500 dark:text-gray-400">
        {chartData.monthLabels.map((label, idx) => (
          <div
            key={idx}
            className="absolute"
            style={{ left: `${label.weekIndex * 16 + 32}px` }}
          >
            {label.label}
          </div>
        ))}
      ...
      <div className="flex mt-4">
        {/* Day labels */}
        <div className="flex flex-col gap-0.5 text-xs text-gray-500 dark:text-gray-400 pr-1">
          {DAY_LABELS.map((label, idx) => (
            <div key={idx} className="h-3 flex items-center justify-end" style={{ width: '28px' }}>
              {label}
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-0.5 overflow-x-auto" data-testid={`${testId}-grid`}>
            ...
            <div key={weekIdx} className="flex flex-col gap-0.5">
              {week.map((day, dayIdx) => (
                day.level === -1 ? (
                  ...
                  <div
                    key={dayIdx}
                    className={`${getActivityClass(day.level)} cursor-pointer hover:ring-1 hover:ring-gray-400 dark:hover:ring-gray-500`}
      ...
      <div className="flex items-center justify-between mt-3">
        <div className="text-xs text-gray-500 dark:text-gray-400" data-testid={`${testId}-total`}>
          {chartData.totalActivity} contributions in the last year
        </div>
        <div className="flex items-center gap-1">
          ...
          {[0, 1, 2, 3, 4].map((level) => (
            <div
              key={level}
              className={getActivityClass(level)}
