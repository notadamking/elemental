  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        {/* Loading text */}
        <h2 className="text-lg font-medium text-gray-900 mb-2">Loading Elemental</h2>
...
function DataPreloaderInner({ children }: { children: ReactNode }) {
  ...
  useRealtimeEvents({
    ...
    onEvent: (event: WebSocketEvent) => {
      ...
      const handled = handleInPlaceUpdate(event);
      // If not handled in-place, the default invalidation in useRealtimeEvents will kick in
      // which is fine for edge cases
      if (handled) {
        // Optionally log for debugging
        // console.log('[DataPreloader] Handled event in-place:', event.elementType, event.eventType);
      }
    },
