 ...
 *
 * Manages WebSocket connection to the Elemental platform server.
 * Handles reconnection, subscription management, and event dispatch.
 */
...
export interface WebSocketOptions {
  /** WebSocket URL (default: ws://localhost:3456/ws) */
  url?: string;
 ...
 * WebSocket Connection Manager
 *
 * Singleton class that manages the WebSocket connection.
 */
class WebSocketManager {
  private socket: WebSocket | null = null;
  private url: string;
  private channels: Set<SubscriptionChannel> = new Set();
  ...
  constructor(options: WebSocketOptions = {}) {
    this.url = options.url ?? 'ws://localhost:3456/ws';
   ...
   */
  private sendSubscribe(channels: SubscriptionChannel[]): void {
  ...
  private handleMessage(data: string): void {
    try {
      ...
      switch (message.type) {
        ...
        case 'subscribed':
          console.log('[ws] Subscribed to:', message.channels.join(', '));
          break;

        case 'unsubscribed':
          console.log('[ws] Unsubscribed from:', message.channels.join(', '));
          break;
...
/**
 * Initialize and connect the WebSocket manager
 */
export function initializeWebSocket(options?: WebSocketOptions): WebSocketManager {
