...
export function useDeepLink<T>(options: UseDeepLinkOptions<T>): UseDeepLinkResult {
  ...
  useEffect(() => {
    ...
    if (autoNavigate && result.page !== currentPage) {
      ...
      navigate({
        to: routePath as '/' | '/tasks' | '/plans' | '/workflows' | '/messages' | '/documents' | '/entities' | '/teams' | '/settings' | '/dashboard' | '/dashboard/task-flow' | '/dashboard/agents' | '/dashboard/dependencies' | '/dashboard/timeline',
        search: (prev) => ({
          ...prev,
          page: result.page,
          selected: selectedId,
