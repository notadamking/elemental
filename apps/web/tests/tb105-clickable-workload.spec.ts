...
test.describe('TB105: Clickable Workload Distribution', () => {
  ...
  test.describe('Agent Activity Workload Chart', () => {
    test('workload distribution bars are clickable', async ({ page }) => {
      ...
      const entityId = testId?.replace('workload-bar-', '');
    ...
    test('workload distribution bars show hover effect', async ({ page }) => {
      // Navigate to agent activity page
      await page.goto('/dashboard/agents');
      await page.waitForLoadState('networkidle');
      ...
      // Verify the bar has cursor-pointer class for clickability indicator
      await expect(workloadBar).toHaveClass(/cursor-pointer/);

      // Hover and verify text changes color
      await workloadBar.hover();
      await page.waitForTimeout(100);

      // The name span should change color on hover (group-hover:text-blue-600)
      const nameSpan = workloadBar.locator('span').first();
      // This will work if the hover effect is properly applied
      await expect(nameSpan).toBeVisible();
    });

    test('workload bars display count and percentage', async ({ page }) => {
      // Navigate to agent activity page
      await page.goto('/dashboard/agents');
      await page.waitForLoadState('networkidle');
      ...
      expect(barText).toMatch(/\d+ tasks?/);
