...
test.describe('TB77: Dashboard Quick Actions with Modals', () => {
  ...
  test.describe('Pour Workflow Button', () => {
    test('pour workflow button opens modal instead of navigating', async ({ page }) => {
    ...
    test('pour workflow button shows keyboard shortcut hint (C W)', async ({ page }) => {
      const button = page.getByTestId('quick-action-pour-workflow');
      await expect(button).toContainText('C W');
    });

    test('pour workflow modal can be closed via close button', async ({ page }) => {
      await page.getByTestId('quick-action-pour-workflow').click();
      await expect(page.getByTestId('pour-workflow-modal')).toBeVisible();
    ...
    test('pour workflow modal can be closed via backdrop click', async ({ page }) => {
      await page.getByTestId('quick-action-pour-workflow').click();
      await expect(page.getByTestId('pour-workflow-modal')).toBeVisible();

      // Click outside the modal content (on the backdrop)
      await page.locator('[data-testid="pour-workflow-modal"]').click({ position: { x: 10, y: 10 } });
      await expect(page.getByTestId('pour-workflow-modal')).not.toBeVisible();
    });
  ...
  test.describe('Keyboard Shortcuts', () => {
    ...
    test('C W keyboard shortcut opens pour workflow modal', async ({ page }) => {
    ...
    test('keyboard shortcuts are disabled when create task modal is open', async ({ page }) => {
      ...
      // Try to open pour workflow with shortcut - should not work
      await page.keyboard.press('c');
      await page.keyboard.press('w');

      // Pour modal should NOT be visible (shortcuts disabled)
      await expect(page.getByTestId('pour-workflow-modal')).not.toBeVisible();
    ...
    test('keyboard shortcuts are disabled when pour workflow modal is open', async ({ page }) => {
      // Open pour workflow modal
      await page.getByTestId('quick-action-pour-workflow').click();
      await expect(page.getByTestId('pour-workflow-modal')).toBeVisible();
      ...
      // Pour modal should still be open
      await expect(page.getByTestId('pour-workflow-modal')).toBeVisible();
    });
  });
  ...
  test.describe('Toast Notifications', () => {
    ...
    test('toast appears after successful workflow creation', async ({ page }) => {
      await page.getByTestId('quick-action-pour-workflow').click();
      await page.getByTestId('pour-title-input').fill('Toast Test Workflow');
      await page.getByTestId('pour-submit-button').click();
    ...
    test('toast has View Workflow action button after workflow creation', async ({ page }) => {
      await page.getByTestId('quick-action-pour-workflow').click();
      await page.getByTestId('pour-title-input').fill('Toast Action Test Workflow');
      await page.getByTestId('pour-submit-button').click();
