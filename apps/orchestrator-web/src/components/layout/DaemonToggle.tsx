/**
 * DaemonToggle - Toggle button for dispatch daemon control
 *
 * Displays daemon status and allows starting/stopping the dispatch daemon.
    ...
    return (
      <Tooltip content={status?.reason || 'Daemon unavailable'} side="bottom">
        <div className="flex items-center gap-2 px-2 py-1.5 text-[var(--color-text-tertiary)] cursor-not-allowed">
          ...
          <span className="hidden lg:inline text-sm">Daemon</span>
        </div>
      </Tooltip>
    );
  }
