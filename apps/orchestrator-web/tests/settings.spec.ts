...
test.describe('TB-O25c: Settings Page', () => {
  ...
  test.describe('Preferences Tab - Keyboard Shortcuts', () => {
    ...
    test('displays command palette shortcut', async ({ page }) => {
      ...
      await expect(page.getByTestId('shortcut-row-command-palette')).toBeVisible();
      // Check for the keyboard shortcut text
      await expect(page.getByTestId('shortcut-row-command-palette').locator('kbd')).toContainText('K');
    });
    ...
    test('displays all expected shortcuts', async ({ page }) => {
      ...
      await expect(page.getByTestId('shortcut-row-command-palette')).toBeVisible();
      await expect(page.getByTestId('shortcut-row-toggle-sidebar')).toBeVisible();
      await expect(page.getByTestId('shortcut-row-toggle-director-panel')).toBeVisible();
    });
  });
