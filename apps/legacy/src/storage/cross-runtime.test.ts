 ...
 *
 * These tests verify that storage backends behave consistently across runtimes.
 * The test suite runs against multiple backend implementations to ensure:
 *
 * 1. Interface contract consistency - all backends implement the same behavior
 * 2. Data consistency - same operations produce same results
 * 3. SQL compatibility - SQLite behavior is consistent across implementations
 *
 * Currently tested backends:
 * - Bun (bun:sqlite) - Native Bun SQLite bindings
 * - Browser (sql.js) - WebAssembly-based SQLite
 *
 * Note: Node.js backend (better-sqlite3) is tested separately via vitest
 * due to native module differences. See tests/node/cross-runtime.test.ts
 */
...
const backends: BackendFactory[] = [
  {
    name: 'Bun',
    create: async () => new BunStorageBackend({ path: ':memory:' }),
  },
  {
    name: 'Browser (sql.js)',
    create: async () => BrowserStorageBackend.create({ path: ':memory:' }),
  },
];

// ============================================================================
// Cross-Runtime Compatibility Test Suite
// ============================================================================

describe('Cross-Runtime Compatibility', () => {
  describe.each(backends)('$name backend', ({ name: _name, create }) => {
    ...
    describe('SQL operations', () => {
      ...
      it('should queryOne return undefined for no match', () => {
        ...
        // Different backends may return null or undefined
    ...
    describe('dirty tracking', () => {
      ...
      it('should update timestamp on re-mark', async () => {
        backend.markDirty('el-test');
...
// ============================================================================
// Cross-Backend Data Consistency Tests
// ============================================================================

describe('Cross-Backend Data Consistency', () => {
  it('should produce identical query results across backends', async () => {
    // Create both backends
    const bunBackend = new BunStorageBackend({ path: ':memory:' });
    const browserBackend = await BrowserStorageBackend.create({ path: ':memory:' });

    try {
      // Apply identical schema
      const schema = `
        CREATE TABLE test (
          id INTEGER PRIMARY KEY,
          name TEXT NOT NULL,
      ...
      `;
      bunBackend.exec(schema);
      browserBackend.exec(schema);
      ...
      for (const [name, value, active] of testData) {
        bunBackend.run('INSERT INTO test (name, value, active) VALUES (?, ?, ?)', [
          name,
          value,
          active,
        ]);
        browserBackend.run('INSERT INTO test (name, value, active) VALUES (?, ?, ?)', [
          name,
          value,
          active,
        ]);
      ...
      const bunResult = bunBackend.query('SELECT * FROM test ORDER BY id');
      const browserResult = browserBackend.query('SELECT * FROM test ORDER BY id');

      expect(bunResult).toEqual(browserResult);

      // Compare filtered results
      const bunFiltered = bunBackend.query('SELECT name FROM test WHERE active = 1 ORDER BY name');
      const browserFiltered = browserBackend.query(
        'SELECT name FROM test WHERE active = 1 ORDER BY name'
      );

      expect(bunFiltered).toEqual(browserFiltered);

      // Compare aggregation results
      const bunSum = bunBackend.queryOne<{ total: number }>('SELECT SUM(value) as total FROM test');
      const browserSum = browserBackend.queryOne<{ total: number }>(
        'SELECT SUM(value) as total FROM test'
      );

      expect(bunSum?.total).toBe(browserSum?.total);
    } finally {
      bunBackend.close();
  ...
  it('should handle migrations identically across backends', async () => {
    const bunBackend = new BunStorageBackend({ path: ':memory:' });
    const browserBackend = await BrowserStorageBackend.create({ path: ':memory:' });

    try {
      const migrations: Migration[] = [
        { version: 1, description: 'Create users', up: 'CREATE TABLE users (id INTEGER PRIMARY KEY)' },
        { version: 2, description: 'Add name', up: 'ALTER TABLE users ADD COLUMN name TEXT' },
        { version: 3, description: 'Add email', up: 'ALTER TABLE users ADD COLUMN email TEXT' },
      ];

      const bunResult = bunBackend.migrate(migrations);
      const browserResult = browserBackend.migrate(migrations);

      expect(bunResult.fromVersion).toBe(browserResult.fromVersion);
      expect(bunResult.toVersion).toBe(browserResult.toVersion);
      expect(bunResult.applied).toEqual(browserResult.applied);
      expect(bunResult.success).toBe(browserResult.success);

      expect(bunBackend.getSchemaVersion()).toBe(browserBackend.getSchemaVersion());
    } finally {
      bunBackend.close();
      browserBackend.close();
    }
  ...
  it('should handle dirty tracking identically across backends', async () => {
    const bunBackend = new BunStorageBackend({ path: ':memory:' });
    const browserBackend = await BrowserStorageBackend.create({ path: ':memory:' });

    try {
      // Mark same elements dirty
      const elements = ['el-aaa', 'el-bbb', 'el-ccc'];
      for (const id of elements) {
        bunBackend.markDirty(id);
        browserBackend.markDirty(id);
      }

      const bunDirty = bunBackend.getDirtyElements().map((d) => String(d.elementId)).sort();
      const browserDirty = browserBackend.getDirtyElements().map((d) => String(d.elementId)).sort();

      expect(bunDirty).toEqual(browserDirty);

      // Clear specific elements
      bunBackend.clearDirtyElements(['el-aaa', 'el-ccc']);
      browserBackend.clearDirtyElements(['el-aaa', 'el-ccc']);

      const bunRemaining = bunBackend.getDirtyElements().map((d) => String(d.elementId));
      const browserRemaining = browserBackend.getDirtyElements().map((d) => String(d.elementId));

      expect(bunRemaining).toEqual(browserRemaining);
    } finally {
      bunBackend.close();
      browserBackend.close();
    }
