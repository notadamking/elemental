...
import {
  ...
  isEphemeralTask,
  isDurableTask,
  filterEphemeralTasks,
  filterDurableTasks,
  isTaskEligibleForGarbageCollection,
  filterTasksEligibleForGarbageCollection,
  filterTaskGarbageCollectionByAge,
} from './task.js';

describe('Ephemeral Task Support', () => {
  describe('createTask with ephemeral flag', () => {
    test('creates durable task by default', async () => {
      const task = await createTask({
        title: 'Durable task',
    ...
    test('creates ephemeral task when specified', async () => {
      const task = await createTask({
    ...
    test('creates durable task when explicitly set to false', async () => {
      const task = await createTask({
        title: 'Explicitly durable task',
        createdBy: 'el-user1' as EntityId,
        ephemeral: false,
      });
      expect(task.ephemeral).toBe(false);
  ...
  describe('promoteTask', () => {
    test('promotes ephemeral task to durable', () => {
      const ephemeralTask = createTestTask({ ephemeral: true });
      const promotedTask = promoteTask(ephemeralTask);

      expect(promotedTask.ephemeral).toBe(false);
      expect(promotedTask.updatedAt).not.toBe(ephemeralTask.updatedAt);
    });

    test('throws when promoting already durable task', () => {
      const durableTask = createTestTask({ ephemeral: false });

      expect(() => promoteTask(durableTask)).toThrow(ValidationError);
      expect(() => promoteTask(durableTask)).toThrow('Task is already durable');
    });

    test('preserves all other task properties', () => {
      const ephemeralTask = createTestTask({
        ephemeral: true,
        title: 'My ephemeral task',
        status: TaskStatus.IN_PROGRESS,
        priority: Priority.HIGH,
        assignee: 'el-worker1' as EntityId,
        tags: ['urgent', 'feature'],
      });

      const promotedTask = promoteTask(ephemeralTask);

      expect(promotedTask.id).toBe(ephemeralTask.id);
      expect(promotedTask.title).toBe('My ephemeral task');
      expect(promotedTask.status).toBe(TaskStatus.IN_PROGRESS);
      expect(promotedTask.priority).toBe(Priority.HIGH);
      expect(promotedTask.assignee).toBe('el-worker1');
      expect(promotedTask.tags).toEqual(['urgent', 'feature']);
    });
  });

  describe('isEphemeralTask and isDurableTask', () => {
    test('isEphemeralTask returns true for ephemeral tasks', () => {
      const task = createTestTask({ ephemeral: true });
      expect(isEphemeralTask(task)).toBe(true);
    });

    test('isEphemeralTask returns false for durable tasks', () => {
      const task = createTestTask({ ephemeral: false });
      expect(isEphemeralTask(task)).toBe(false);
    });

    test('isDurableTask returns true for durable tasks', () => {
      const task = createTestTask({ ephemeral: false });
      expect(isDurableTask(task)).toBe(true);
    });

    test('isDurableTask returns false for ephemeral tasks', () => {
      const task = createTestTask({ ephemeral: true });
      expect(isDurableTask(task)).toBe(false);
    });
  });

  describe('filterEphemeralTasks and filterDurableTasks', () => {
    test('filterEphemeralTasks returns only ephemeral tasks', () => {
      const tasks = [
        createTestTask({ id: 'el-1' as ElementId, ephemeral: true }),
        createTestTask({ id: 'el-2' as ElementId, ephemeral: false }),
        createTestTask({ id: 'el-3' as ElementId, ephemeral: true }),
        createTestTask({ id: 'el-4' as ElementId, ephemeral: false }),
      ];

      const ephemeral = filterEphemeralTasks(tasks);
      expect(ephemeral).toHaveLength(2);
      expect(ephemeral.map((t) => t.id)).toEqual(['el-1', 'el-3']);
    });

    test('filterDurableTasks returns only durable tasks', () => {
      const tasks = [
        createTestTask({ id: 'el-1' as ElementId, ephemeral: true }),
        createTestTask({ id: 'el-2' as ElementId, ephemeral: false }),
        createTestTask({ id: 'el-3' as ElementId, ephemeral: true }),
      ...
      const durable = filterDurableTasks(tasks);
      expect(durable).toHaveLength(2);
      expect(durable.map((t) => t.id)).toEqual(['el-2', 'el-4']);
    });

    test('returns empty array when no matching tasks', () => {
      const allDurable = [
        createTestTask({ id: 'el-1' as ElementId, ephemeral: false }),
        createTestTask({ id: 'el-2' as ElementId, ephemeral: false }),
      ];
      const allEphemeral = [
        createTestTask({ id: 'el-3' as ElementId, ephemeral: true }),
        createTestTask({ id: 'el-4' as ElementId, ephemeral: true }),
      ];

      expect(filterEphemeralTasks(allDurable)).toHaveLength(0);
      expect(filterDurableTasks(allEphemeral)).toHaveLength(0);
    });
  });

  describe('Garbage Collection eligibility', () => {
    test('isTaskEligibleForGarbageCollection returns true for closed ephemeral tasks', () => {
      const task = createTestTask({
        ephemeral: true,
        status: TaskStatus.CLOSED,
        closedAt: '2025-01-22T12:00:00.000Z' as Timestamp,
      });
      expect(isTaskEligibleForGarbageCollection(task)).toBe(true);
    });

    test('isTaskEligibleForGarbageCollection returns true for tombstoned ephemeral tasks', () => {
      const task = createTestTask({
        ephemeral: true,
        status: TaskStatus.TOMBSTONE,
        deletedAt: '2025-01-22T12:00:00.000Z' as Timestamp,
      });
      expect(isTaskEligibleForGarbageCollection(task)).toBe(true);
    });

    test('isTaskEligibleForGarbageCollection returns false for open ephemeral tasks', () => {
      const task = createTestTask({
        ephemeral: true,
        status: TaskStatus.OPEN,
      });
      expect(isTaskEligibleForGarbageCollection(task)).toBe(false);
    });

    test('isTaskEligibleForGarbageCollection returns false for in_progress ephemeral tasks', () => {
      const task = createTestTask({
        ephemeral: true,
        status: TaskStatus.IN_PROGRESS,
      });
    ...
    test('isTaskEligibleForGarbageCollection returns false for durable closed tasks', () => {
      const task = createTestTask({
        ephemeral: false,
        status: TaskStatus.CLOSED,
        closedAt: '2025-01-22T12:00:00.000Z' as Timestamp,
    ...
    test('filterTasksEligibleForGarbageCollection filters correctly', () => {
      const tasks = [
        // Eligible: ephemeral + closed
        createTestTask({
          id: 'el-1' as ElementId,
          ephemeral: true,
          status: TaskStatus.CLOSED,
        }),
        // Eligible: ephemeral + tombstone
        createTestTask({
          id: 'el-2' as ElementId,
          ephemeral: true,
          status: TaskStatus.TOMBSTONE,
        }),
        // Not eligible: ephemeral + open
        createTestTask({
          id: 'el-3' as ElementId,
          ephemeral: true,
          status: TaskStatus.OPEN,
        }),
        // Not eligible: durable + closed
        createTestTask({
      ...
      const eligible = filterTasksEligibleForGarbageCollection(tasks);
      expect(eligible).toHaveLength(2);
      expect(eligible.map((t) => t.id)).toEqual(['el-1', 'el-2']);
    });
  });

  describe('Garbage Collection by age', () => {
    test('filters tasks older than maxAgeMs', () => {
      const now = Date.now();
      const oneHourAgo = new Date(now - 60 * 60 * 1000).toISOString() as Timestamp;
      const twoHoursAgo = new Date(now - 2 * 60 * 60 * 1000).toISOString() as Timestamp;
      const fiveMinutesAgo = new Date(now - 5 * 60 * 1000).toISOString() as Timestamp;
      ...
      const tasks = [
        ...
        // Too recent (5 minutes)
        createTestTask({
          id: 'el-3' as ElementId,
          ephemeral: true,
          status: TaskStatus.CLOSED,
          closedAt: fiveMinutesAgo,
        }),
        // Durable - never eligible
        createTestTask({
          id: 'el-4' as ElementId,
          ephemeral: false,
          status: TaskStatus.CLOSED,
      ...
      // Filter for tasks older than 30 minutes
      const maxAgeMs = 30 * 60 * 1000;
      const eligible = filterTaskGarbageCollectionByAge(tasks, maxAgeMs);

      expect(eligible).toHaveLength(2);
      expect(eligible.map((t) => t.id)).toEqual(['el-1', 'el-2']);
    });

    test('uses deletedAt for tombstoned tasks', () => {
      const now = Date.now();
      const oneHourAgo = new Date(now - 60 * 60 * 1000).toISOString() as Timestamp;
      ...
      // 30 minute threshold - should be eligible
      const eligible = filterTaskGarbageCollectionByAge([task], 30 * 60 * 1000);
      expect(eligible).toHaveLength(1);

      // 2 hour threshold - should not be eligible
      const notEligible = filterTaskGarbageCollectionByAge([task], 2 * 60 * 60 * 1000);
      expect(notEligible).toHaveLength(0);
    });
  });

  describe('isTask validation', () => {
    test('rejects task without ephemeral field', () => {
      const taskWithoutEphemeral = {
        id: 'el-abc123',
        type: ElementType.TASK,
        ...
        complexity: Complexity.MEDIUM,
        taskType: TaskTypeValue.TASK,
        // Missing ephemeral field
      };

      expect(isTask(taskWithoutEphemeral)).toBe(false);
    });

    test('rejects task with non-boolean ephemeral', () => {
      const taskWithBadEphemeral = {
        ...createTestTask(),
        ephemeral: 'true', // string instead of boolean
      };

      expect(isTask(taskWithBadEphemeral)).toBe(false);
    });

    test('accepts task with ephemeral true', () => {
      const task = createTestTask({ ephemeral: true });
      expect(isTask(task)).toBe(true);
    });

    test('accepts task with ephemeral false', () => {
      const task = createTestTask({ ephemeral: false });
      expect(isTask(task)).toBe(true);
    });
  });

  describe('validateTask validation', () => {
    test('throws for task without ephemeral field', () => {
      const taskWithoutEphemeral = {
        id: 'el-abc123',
        type: ElementType.TASK,
        createdAt: '2025-01-22T10:00:00.000Z',
      ...
      expect(() => validateTask(taskWithoutEphemeral)).toThrow(ValidationError);
      expect(() => validateTask(taskWithoutEphemeral)).toThrow('Task ephemeral must be a boolean');
    });

    test('throws for task with non-boolean ephemeral', () => {
      const taskWithBadEphemeral = {
        ...createTestTask(),
        ephemeral: 'yes',
      };

      expect(() => validateTask(taskWithBadEphemeral)).toThrow(ValidationError);
    });
  });
});
