...
describe('workflow lifecycle scenarios', () => {
  ...
  test('workflow status transitions via API', async () => {
    ...
    await api.update(workflow.id as import('../../types/element.js').ElementId, { status: WorkflowStatus.RUNNING });
    ...
    await api.update(workflow.id as import('../../types/element.js').ElementId, { status: WorkflowStatus.COMPLETED });
