...
export function generateBashCompletion(commands: Command[]): string {
  ...
  return `# Bash completion for elemental (el)
# Add to ~/.bashrc or ~/.bash_completion:
#   source <(el completion bash)

_elemental_completion() {
...
complete -F _elemental_completion el
complete -F _elemental_completion elemental
`;
}
...
${opts}
  ...
  return `#compdef el elemental
# Zsh completion for elemental (el)
...
${cmdDescs}
    ...
    _describe -t commands 'el command' commands
}
...
export function generateFishCompletion(commands: Command[]): string {
  const lines: string[] = [
    '# Fish completion for elemental (el)',
    '# Add to ~/.config/fish/completions/el.fish:',
    '#   el completion fish > ~/.config/fish/completions/el.fish',
    '',
    '# Disable file completion by default',
    'complete -c el -f',
    'complete -c elemental -f',
    '',
    '# Global options',
  ];
  ...
  for (const opt of GLOBAL_OPTIONS) {
    ...
    lines.push(`complete -c el ${short}-l ${opt.name} ${requiresArg}-d '${escapeFish(opt.description)}'`);
    lines.push(`complete -c elemental ${short}-l ${opt.name} ${requiresArg}-d '${escapeFish(opt.description)}'`);
  }
  ...
  for (const cmd of commands) {
    lines.push(`complete -c el -n '__fish_use_subcommand' -a ${cmd.name} -d '${escapeFish(cmd.description)}'`);
    lines.push(`complete -c elemental -n '__fish_use_subcommand' -a ${cmd.name} -d '${escapeFish(cmd.description)}'`);

    // Subcommands
    if (cmd.subcommands) {
      lines.push('', `# ${cmd.name} subcommands`);
      for (const [subName, sub] of Object.entries(cmd.subcommands)) {
        lines.push(`complete -c el -n '__fish_seen_subcommand_from ${cmd.name}' -a ${subName} -d '${escapeFish(sub.description)}'`);
        lines.push(`complete -c elemental -n '__fish_seen_subcommand_from ${cmd.name}' -a ${subName} -d '${escapeFish(sub.description)}'`);
      }
    }
    ...
    if (cmd.options && cmd.options.length > 0) {
      ...
      for (const opt of cmd.options) {
        const short = opt.short ? `-s ${opt.short} ` : '';
        const requiresArg = opt.hasValue ? '-r ' : '';
        lines.push(`complete -c el -n '__fish_seen_subcommand_from ${cmd.name}' ${short}-l ${opt.name} ${requiresArg}-d '${escapeFish(opt.description)}'`);
        lines.push(`complete -c elemental -n '__fish_seen_subcommand_from ${cmd.name}' ${short}-l ${opt.name} ${requiresArg}-d '${escapeFish(opt.description)}'`);
      }
    }
  }
