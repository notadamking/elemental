...
import type { EventFilter, EventType } from '../../types/event.js';
...
async function historyHandler(
  ...
  if (!id) {
    return failure('Usage: el history <id>', ExitCode.INVALID_ARGUMENTS);
  }
...
Examples:
  el history el-abc123
  el history el-abc123 --limit 10
  el history el-abc123 --type updated
  el history el-abc123 --actor user:alice
  el history el-abc123 --format table
  el history el-abc123 --after 2024-01-01
  el history el-abc123 --json`,
