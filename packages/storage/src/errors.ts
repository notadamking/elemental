 ...
 *
 * Maps SQLite error codes and messages to Elemental error types.
 * This provides a consistent error interface regardless of the
 * underlying SQLite backend being used.
 */

import { StorageError, ConflictError, ConstraintError, ErrorCode } from '@elemental/core';
...
/**
 * Convert a SQLite error to an appropriate Elemental error type
 *
 * @param error - The original SQLite error
 * @param context - Optional context about the operation that failed
 * @returns An ElementalError subclass with appropriate code and details
 */
export function mapStorageError(
  ...
  // Already an Elemental error, return as-is
