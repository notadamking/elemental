...
describe('ElementalError', () => {
  describe('constructor', () => {
    it('should create error with required parameters', () => {
      const error = new ElementalError('Test error', ErrorCode.INVALID_INPUT);

      expect(error.message).toBe('Test error');
      expect(error.code).toBe(ErrorCode.INVALID_INPUT);
      expect(error.details).toEqual({});
      expect(error.name).toBe('ElementalError');
      expect(error.httpStatus).toBe(400);
    });

    it('should create error with details', () => {
      const details = { field: 'title', value: 'too long' };
      const error = new ElementalError('Test error', ErrorCode.TITLE_TOO_LONG, details);
    ...
    it('should create error with cause', () => {
      ...
      const error = new ElementalError('Wrapped error', ErrorCode.DATABASE_ERROR, {}, cause);

      expect(error.cause).toBe(cause);
    });

    it('should extend Error', () => {
      const error = new ElementalError('Test', ErrorCode.NOT_FOUND);
      expect(error).toBeInstanceOf(Error);
    });

    it('should have correct HTTP status for each error code', () => {
      for (const code of Object.values(ErrorCode)) {
        const error = new ElementalError('Test', code);
  ...
  describe('toJSON', () => {
    it('should serialize error to JSON', () => {
      const error = new ElementalError(
        'Element not found',
        ErrorCode.NOT_FOUND,
        { elementId: 'el-abc123' }
      );
    ...
    it('should produce JSON-serializable output', () => {
      const error = new ElementalError('Test', ErrorCode.INVALID_INPUT, {
        value: { nested: true },
      });
...
describe('ValidationError', () => {
  it('should create validation error with default code', () => {
    ...
    expect(error).toBeInstanceOf(ElementalError);
  ...
  it('should inherit from ElementalError', () => {
...
describe('Type Guards', () => {
  describe('isElementalError', () => {
    it('should return true for ElementalError', () => {
      expect(isElementalError(new ElementalError('Test', ErrorCode.NOT_FOUND))).toBe(true);
    });

    it('should return true for error subclasses', () => {
      expect(isElementalError(new ValidationError('Test'))).toBe(true);
      expect(isElementalError(new NotFoundError('Test'))).toBe(true);
      expect(isElementalError(new ConflictError('Test', ErrorCode.ALREADY_EXISTS))).toBe(true);
      expect(isElementalError(new ConstraintError('Test', ErrorCode.IMMUTABLE))).toBe(true);
      expect(isElementalError(new StorageError('Test'))).toBe(true);
    });

    it('should return false for regular Error', () => {
      expect(isElementalError(new Error('Test'))).toBe(false);
    });

    it('should return false for non-error values', () => {
      expect(isElementalError(null)).toBe(false);
      expect(isElementalError(undefined)).toBe(false);
      expect(isElementalError('error')).toBe(false);
      expect(isElementalError({ message: 'Test', code: 'NOT_FOUND' })).toBe(false);
    });
  });
  ...
  describe('isValidationError', () => {
    ...
    it('should return false for other error types', () => {
      ...
      expect(isValidationError(new ElementalError('Test', ErrorCode.NOT_FOUND))).toBe(false);
  ...
  describe('isNotFoundError', () => {
    ...
    it('should return false for other error types', () => {
      ...
      expect(isNotFoundError(new ElementalError('Test', ErrorCode.NOT_FOUND))).toBe(false);
    });
  });
  ...
  describe('hasErrorCode', () => {
    ...
    it('should return false for non-ElementalError', () => {
...
describe('Error hierarchy', () => {
  it('should maintain proper inheritance chain', () => {
    ...
    // All should be instances of Error and ElementalError
