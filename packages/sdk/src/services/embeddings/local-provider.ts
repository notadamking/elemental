 ...
 * Model is downloaded on first use via `el embeddings install`.
 *
...
const DEFAULT_MODEL_DIR = '.elemental/models';
...
export class LocalEmbeddingProvider implements EmbeddingProvider {
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    if (!available) {
      throw new Error(
        `Local embedding model not installed. Run 'el embeddings install' to download the model.`
      );
    }
