...
import { ValidationError, ErrorCode } from '@elemental/core';
...
/** Default .elemental directory name */
export const ELEMENTAL_DIR = '.elemental';
...
export function findElementalDir(startDir: string): string | undefined {
  ...
  while (currentDir !== root) {
    const elementalPath = path.join(currentDir, ELEMENTAL_DIR);
  ...
  // Check root
  const rootElementalPath = path.join(root, ELEMENTAL_DIR);
  if (fs.existsSync(rootElementalPath) && fs.statSync(rootElementalPath).isDirectory()) {
    return rootElementalPath;
  }
...
export function getGlobalConfigDir(): string {
  return path.join(os.homedir(), ELEMENTAL_DIR);
}
...
export function discoverConfigFile(
  ...
  if (overridePath) {
    ...
    return {
      path: resolvedPath,
      exists,
      elementalDir: exists ? path.dirname(resolvedPath) : undefined,
    };
  }

  // Try to find .elemental directory
  const elementalDir = findElementalDir(startDir);
  if (elementalDir) {
  ...
  return {
    ...
    elementalDir: fs.existsSync(getGlobalConfigDir()) ? getGlobalConfigDir() : undefined,
  };
}
...
export function writeConfigFile(
  ...
  const content = `# Elemental Configuration\n\n${serializeConfigToYaml(config)}`;
