...
import type { Team } from '@elemental/core';
import type { Element, EntityId } from '@elemental/core';
...
async function createTestEntity(
  ...
  const { createStorage, initializeSchema } = await import('@elemental/storage');
  const { createEntity } = await import('@elemental/core');
...
describe('team add command', () => {
  ...
  test('fails when adding non-entity element (task) to team', async () => {
    ...
    const { createElementalAPI } = await import('../../api/elemental-api.js');
    const { createStorage, initializeSchema } = await import('@elemental/storage');
    const { createTask } = await import('@elemental/core');
    const backend = createStorage({ path: DB_PATH, create: true });
    initializeSchema(backend);
    const api = createElementalAPI(backend);
  ...
  test('fails when adding non-entity element (document) to team', async () => {
    ...
    const { createElementalAPI } = await import('../../api/elemental-api.js');
    const { createStorage, initializeSchema } = await import('@elemental/storage');
    const { createDocument, ContentType } = await import('@elemental/core');
    const backend = createStorage({ path: DB_PATH, create: true });
    initializeSchema(backend);
    const api = createElementalAPI(backend);
...
describe('team task assignment workflows', () => {
  test('tasks assigned to team appear in ready query for members', async () => {
    ...
    const { createElementalAPI } = await import('../../api/elemental-api.js');
    const { createStorage, initializeSchema } = await import('@elemental/storage');
    const { createTeam } = await import('@elemental/core');
    const { createTask } = await import('@elemental/core');
    const backend = createStorage({ path: DB_PATH, create: true });
    initializeSchema(backend);
    const api = createElementalAPI(backend);
  ...
  test('team metrics reflect task completion', async () => {
    ...
    const { createStorage, initializeSchema } = await import('@elemental/storage');
    const { createTeam } = await import('@elemental/core');
    const { createTask, TaskStatus } = await import('@elemental/core');
    const backend = createStorage({ path: DB_PATH, create: true });
    initializeSchema(backend);
    const api = createElementalAPI(backend);
