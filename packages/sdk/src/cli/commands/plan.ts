...
import { createPlan, PlanStatus, type CreatePlanInput, type Plan } from '@elemental/core';
...
const planCreateCommand: Command = {
  ...
  usage: 'el plan create --title <title> [options]',
...
Examples:
  el plan create --title "Q1 Feature Roadmap"
  el plan create -t "Sprint 3" --status active
  el plan create --title "Bug Backlog" --tag urgent --tag backend`,
...
Examples:
  el plan list
  el plan list --status active
  el plan list --tag sprint --tag q1`,
...
Examples:
  el plan show el-abc123
  el plan show el-abc123 --tasks
  el plan show el-abc123 --json`,
...
const planCancelCommand: Command = {
  ...
  usage: 'el plan cancel <id> [options]',
...
Examples:
  el plan cancel el-abc123
  el plan cancel el-abc123 --reason "Requirements changed"`,
...
const planAddTaskCommand: Command = {
  ...
  usage: 'el plan add-task <plan-id> <task-id>',
...
Examples:
  el plan add-task el-plan123 el-task456`,
...
const planRemoveTaskCommand: Command = {
  ...
  usage: 'el plan remove-task <plan-id> <task-id>',
...
Examples:
  el plan remove-task el-plan123 el-task456`,
...
const planTasksCommand: Command = {
  ...
  usage: 'el plan tasks <plan-id> [options]',
...
Examples:
  el plan tasks el-abc123
  el plan tasks el-abc123 --status open`,
...
export const planCommand: Command = {
  ...
  usage: 'el plan <subcommand> [options]',
...
Examples:
  el plan create --title "Q1 Roadmap"
  el plan list --status active
  el plan show el-abc123 --tasks
  el plan activate el-abc123
  el plan add-task el-plan123 el-task456`,
  ...
  handler: async (args, options): Promise<CommandResult> => {
    ...
    return failure(
      `Unknown subcommand: ${args[0]}. Use 'el plan --help' for available subcommands.`,
