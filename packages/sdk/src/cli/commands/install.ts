/**
 * Install Commands - Install elemental extensions to the workspace
 *
...
function findSkillsSourceDir(): string | null {
  ...
  const nodeModulesPath = join(process.cwd(), 'node_modules', '@elemental', 'orchestrator-sdk');
  ...
  const globalNodeModulesPath = join(dirname(process.execPath), '..', 'lib', 'node_modules', '@elemental', 'orchestrator-sdk', 'dist', 'skills');
...
async function installSkillsHandler(
  ...
  if (!skillsSourceDir) {
    return failure(
      'Could not find skills to install. Make sure @elemental/orchestrator-sdk is installed.',
...
export const installCommand: Command = {
  name: 'install',
  description: 'Install elemental extensions',
  usage: 'el install <subcommand> [options]',
  help: `Install elemental extensions to the workspace.

Subcommands:
  skills    Install Claude skills to .claude/skills/

Examples:
  el install skills          Install Claude skills to workspace
  el install skills --force  Overwrite existing skills`,
  subcommands: {
  ...
  handler: async (args, _options): Promise<CommandResult> => {
    if (args.length === 0) {
      return failure(
        `Missing subcommand. Use 'el install skills'. Run 'el install --help' for more information.`,
