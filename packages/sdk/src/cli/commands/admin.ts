...
import { createStorage, type StorageBackend } from '@elemental/storage';
import {
  initializeSchema,
  getSchemaVersion,
  isSchemaUpToDate,
...
async function doctorHandler(
  ...
  const elementalDir = join(process.cwd(), ELEMENTAL_DIR);
  const workspaceExists = existsSync(elementalDir);

  if (!workspaceExists && !options.db) {
    diagnostics.push({
      name: 'workspace',
      status: 'error',
      message: 'No .elemental directory found',
      details: { path: elementalDir },
    });
  ...
  diagnostics.push({
    name: 'workspace',
    status: 'ok',
    message: options.db ? `Using custom database path: ${options.db}` : `Workspace found at ${elementalDir}`,
  });
  ...
  if (!dbPath) {
    diagnostics.push({
      ...
      details: { expectedPath: join(elementalDir, DEFAULT_DB_NAME) },
    });
    return buildDoctorResult(diagnostics, options);
  }
...
function buildDoctorResult(
  ...
  if (healthy) {
    ...
    lines.push('');
    lines.push('Issues detected. Run "el migrate" to fix schema issues.');
  }
...
async function migrateHandler(
  ...
  if (!dbPath) {
    return failure(
      'No database found. Run "el init" to initialize a workspace, or specify --db path',
  ...
  if (options.dryRun) {
    ...
    lines.push('');
    lines.push(`Run "el migrate" without --dry-run to apply these migrations.`);
...
- Workspace exists (.elemental directory)
...
Examples:
  el doctor              Run all diagnostics
  el doctor --verbose    Show detailed information
  el doctor --json       Output as JSON`,
...
Examples:
  el migrate             Run pending migrations
  el migrate --dry-run   Preview migrations without applying`,
  options: [
    {
      name: 'dry-run',
