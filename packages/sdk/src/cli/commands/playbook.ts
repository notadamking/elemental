...
const playbookListCommand: Command = {
  ...
  usage: 'el playbook list [options]',
...
async function playbookShowHandler(
  ...
  if (!nameOrId) {
    return failure('Usage: el playbook show <name|id>', ExitCode.INVALID_ARGUMENTS);
  }
...
const playbookShowCommand: Command = {
  ...
  usage: 'el playbook show <name|id> [options]',
...
Examples:
  el playbook show deploy
  el playbook show el-abc123 --steps --variables
  el playbook show deploy --json`,
...
const playbookCreateCommand: Command = {
  ...
  usage: 'el playbook create --name <name> --title <title> [options]',
...
Examples:
  el playbook create --name deploy --title "Deployment Process"
  el playbook create -n deploy -t "Deploy" -s "build:Build app" -s "test:Run tests:build"
  el playbook create -n deploy -t "Deploy" -v "env:string" -v "debug:boolean:false:false"`,
...
export const playbookCommand: Command = {
  ...
  usage: 'el playbook <subcommand> [options]',
...
Examples:
  el playbook list
  el playbook show deploy --steps --variables
  el playbook validate deploy
  el playbook create --name deploy --title "Deployment"`,
  subcommands: {
