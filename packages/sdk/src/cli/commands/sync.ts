...
function resolveSyncDir(options: { output?: string; input?: string }, isExport: boolean): string {
  ...
  // Default to .elemental/sync
  const elementalDir = join(process.cwd(), ELEMENTAL_DIR);
  return join(elementalDir, DEFAULT_SYNC_DIR);
}
...
const exportOptions: CommandOption[] = [
  {
    name: 'output',
    short: 'o',
    description: 'Output directory path (default: .elemental/sync)',
    hasValue: true,
  },
  {
    name: 'full',
...
Options:
  -o, --output <dir>       Output directory (default: .elemental/sync)
...
const importOptions: CommandOption[] = [
  {
    name: 'input',
    short: 'i',
    description: 'Input directory path (default: .elemental/sync)',
    hasValue: true,
  },
  {
    name: 'dry-run',
...
Options:
  -i, --input <dir>    Input directory (default: .elemental/sync)
...
async function statusHandler(
  ...
  try {
    ...
    const syncDir = join(process.cwd(), ELEMENTAL_DIR, DEFAULT_SYNC_DIR);
    ...
    if (dirtyCount > 0) {
      lines.push('');
      lines.push('Run "el export" to export pending changes.');
    }
...
Examples:
  ...
  handler: async (_args, options) => {
    ...
    return failure('Usage: el sync <command>\n\nCommands: export, import, status\n\nRun "el sync --help" for more information.', ExitCode.INVALID_ARGUMENTS);
  },
};
