 ...
 * - el merge                          -- squash-merge current branch into master
 * - el merge --branch feature/xyz     -- squash-merge a specific branch
 * - el merge --cleanup                -- also delete source branch and worktree
 */
...
async function mergeHandler(
  ...
  try {
    ...
    // Find workspace root by looking for .elemental or .git
    let workspaceRoot = cwd;
    const fs = await import('node:fs');
    while (workspaceRoot !== '/') {
      if (
        fs.existsSync(path.join(workspaceRoot, '.elemental')) ||
    ...
    const mergeDir = path.join(workspaceRoot, '.elemental/.worktrees', mergeDirName);
...
Examples:
  el merge
  el merge --message "feat: implement user auth"
