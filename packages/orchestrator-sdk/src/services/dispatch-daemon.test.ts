...
function createMockWorktreeManager(): WorktreeManager {
  return {
    ...
    createMergeWorktree: mock(async (options: { stewardName: string; taskId: string; taskBranch: string }): Promise<CreateWorktreeResult> => ({
      path: `.elemental/.worktrees/${options.stewardName}-merge-${options.taskId.replace(/[^a-zA-Z0-9-]/g, '-')}`,
      relativePath: `.elemental/.worktrees/${options.stewardName}-merge-${options.taskId.replace(/[^a-zA-Z0-9-]/g, '-')}`,
