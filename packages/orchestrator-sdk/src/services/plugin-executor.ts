...
import type { PlaybookId, Timestamp } from '@elemental/core';
...
export class PluginExecutorImpl implements PluginExecutor {
  ...
  constructor(options?: { api?: ElementalAPI; workspaceRoot?: string }) {
  ...
  private async executePlaybook(
    ...
    if (!this.api) {
      return {
        ...
        pluginType: 'playbook',
        success: false,
        error: 'Playbook execution requires ElementalAPI to be provided',
        durationMs: Date.now() - startTime,
        startedAt,
        completedAt: createTimestamp(),
      };
