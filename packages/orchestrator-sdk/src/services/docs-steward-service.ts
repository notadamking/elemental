...
export class DocsStewardServiceImpl implements DocsStewardService {
  ...
  async verifyCliCommands(): Promise<DocIssue[]> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Match CLI command invocations like `el task list` or `el gc workflows`
      const cmdMatch = line.match(/`el\s+([a-z-]+(?:\s+[a-z-]+)?)`/);
      if (cmdMatch) {
        ...
        if (!availableCommands.has(mainCommand)) {
          issues.push({
            ...
            line: lineNum,
            description: `CLI command not found: el ${mainCommand}`,
            currentValue: `el ${command}`,
            confidence: 'high',
            context: this.getContext(lines, i),
            complexity: 'medium',
          });
