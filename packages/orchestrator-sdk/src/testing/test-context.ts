...
export interface TestContext {
  ...
  /** Underlying ElementalAPI */
  readonly elementalApi: ElementalAPI;
...
export async function setupTestContext(
  ...
  try {
    ...
    await mkdir(elementalDir, { recursive: true });
    const dbPath = join(elementalDir, 'elemental.db');
    ...
    const elementalApi = api as unknown as ElementalAPI;
    const systemEntity = await createSystemEntity(elementalApi);
    ...
    const worktreeManager = createWorktreeManager({
      ...
      worktreeDir: '.elemental/.worktrees',
    });
...
async function createProjectStructure(dir: string): Promise<void> {
  ...
  await mkdir(join(dir, '.elemental'), { recursive: true });
 ...
 */
async function createSystemEntity(api: ElementalAPI): Promise<EntityId> {
...
export async function sendTestMessage(
  ...
  const doc = await ctx.elementalApi.create({
    type: 'document',
    kind: 'document',
    subKind: 'message',
    title: 'Test Message',
  ...
  } as unknown as Parameters<ElementalAPI['create']>[0]);

  // Create the message
  await ctx.elementalApi.create({
    type: 'message',
    kind: 'message',
    subKind: 'direct',
    channel: channelId,
